<template>
  <div class="home container-fluid p-4">
    <div class="row my-2">
      <div>
        <button
          class="btn btn-primary mx-3 createButton"
          v-if="this.$auth.isAuthenticated"
          @click="createToggle = !createToggle"
        >
          <i class="fa fa-plus text-success" aria-hidden="true"></i>
          <i class="fa fa-bug text-success" aria-hidden="true"></i>
        </button>
      </div>
      <div class="col-12 col-md-8">
        <form
          class="form row addCard m-0 flex-column"
          @submit.prevent="createBug"
          v-if="createToggle"
        >
          <div class="col-6 form-group p-0">
            <h5 class="text-primary">
              Add <i class="fa fa-bug text-primary" aria-hidden="true"></i>:
            </h5>
            <input
              type="text"
          ...
          <button
            type="submit"
            class="btn btn-primary col-3 logButton text-success"
          >
        ...
        <div class="card bg-dark">
          <div class="card-header">
            ...
            <div class="text-right">
              <button
                class="m-1 btn btn-outline-info allButton"
                @click="
                  showAll = true;
                  showOpen = false;
                  showClosed = false;
                "
              >
                All <i class="fa fa-bug text-info" aria-hidden="true"></i>'s
              </button>
              <button
                class="m-1 btn btn-outline-success openButton"
                @click="
                  showOpen = true;
                  showClosed = false;
                  showAll = false;
                "
              >
                Open <i class="fa fa-bug text-success" aria-hidden="true"></i>'s
              </button>
              <button
                class="m-1 btn btn-outline-danger closedButton"
                @click="
                  showClosed = true;
                  showOpen = false;
                  showAll = false;
              ...
              >
                Closed
                <i class="fa fa-bug text-danger" aria-hidden="true"></i>'s
              </button>
            </div>
          </div>
...
.createButton {
  border-radius: 15px;
  box-shadow: 1px 1px 4px #5881ab, -1px -1px 4px #5881ab;
}
.closedButton {
  border-radius: 15px;
...
.allButton {
  border-radius: 15px;
  box-shadow: 1px 1px 4px #b0e0e6, -1px -1px 4px #b0e0e6;
}
.logButton {
  border-radius: 15px;
  box-shadow: 1px 1px 4px #5881ab, -1px -1px 4px #5881ab;
